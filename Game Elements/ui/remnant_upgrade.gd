...
func popup_upgrade(is_multiplayer_in : bool, player1_remnants_in : Array, player2_remnants_in : Array):
...
func _handle_multiplayer_input(event):
	...
	if event.is_action_pressed("menu_right_0"):
		hover_index = min(upgrade_remnants.size() - 1, hover_index + 1)
		nav_timer = nav_cooldown
	if Input.is_action_just_pressed("activate_0"):
		if _check_if_remnant_viable(upgrade_remnants[hover_index], player2_remnants) and hover_index != selected_index2:
...
func _on_slot_selected(idx: int) -> void:
	if is_multiplayer:
		if _check_if_remnant_viable(upgrade_remnants[idx], player1_remnants) and idx != selected_index2:
			#Purple select
			selected_index1 = idx
	else:
		#Purple select
		if selected_index1 == -1:
			...
			return
		#Orange Select
		if selected_index2 == -1 and idx != selected_index1 and _check_if_remnant_viable(upgrade_remnants[idx], player2_remnants):
			selected_index2 = idx
			#If we now have two different selections -> close the menu
			_close_after_two_chosen()
